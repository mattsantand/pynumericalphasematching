...
from Noise import StandardNoise
    ...
    def set_wavelengths(self, lam_red, lam_green, lam_blue, constlam="shg"):
        ...
        if self.constlam == "b":
            ...
            if lam_green == 0:
                ...
                self.lamg0 = (abs(self.lamb0) ** -1 - abs(self.lamr0) ** -1)
                lam_green = (abs(lam_blue) ** -1 - abs(lam_red) ** -1) ** -1
            elif lam_red == 0:
                self.lamg0 = lam_green.mean()
                self.lamr0 = (abs(self.lamb0) ** -1 - abs(self.lamg0) ** -1)
            ...
            if lam_blue == 0:
                self.lamr0 = lam_red.mean()
                self.lamb0 = (abs(self.lamb0) ** -1 + abs(self.lamr0) ** -1)
                lam_blue = (abs(lam_green) ** -1 + abs(lam_red) ** -1) ** -1
            elif lam_red == 0:
                self.lamb0 = lam_blue.mean()
                self.lamr0 = (abs(self.lamb0) ** -1 - abs(self.lamr0) ** -1)
                lam_red = (abs(lam_blue) ** -1 - abs(lam_green) ** -1) ** -1
            else:
            ...
            if lam_blue == 0:
                ...
                self.lamr0 = lam_red.mean()
                self.lamg0 = (abs(lamb0) ** -1 - abs(self.lamr0) ** -1) ** -1
                lam_green = (abs(lam_blue) ** -1 - abs(lam_red) ** -1) ** -1
            else:
        ...
        print "Saferty check on central wavelengths: ", self.lamb0, self.lamg0, self.lamr0, self.lamb0 ** -1 - (
    ...
    def __calculate_delta_k(self, wl_red, wl_green, wl_blue, n_red, n_green, n_blue):
        ...
        if self.propagation_type == "copropagation":
            dd = 2 * pi * (n_blue(abs(wl_blue) * 1e6) / wl_blue -
                           ...
                           float(self.order) / self.waveguide.poling)
            return dd
...
def phasematching_2D():
    ...
    waveguide2.evaluate_profile(parameter="width",
                                profile="uniform",
                                nominal_parameter=7.4)
    process1 = Phasematching2D(waveguide=waveguide1,
                               n_red=nte,
                               n_green=ntm,
                               n_blue=nte,
                               process="sfg")
    process1.set_red(central_wavelength=1.55e-6,
                     delta_lambda=20e-9,
                     n_points=500)
    process1.set_blue(central_wavelength=0.55e-6,
                      delta_lambda=1e-9,
                      n_points=500)
    process1.calculate_phasematching()
    process1.plot_phasematching()
    process2 = Phasematching2D(waveguide=waveguide2,
                               n_red=nte,
                               n_green=ntm,
                               n_blue=nte,
                               process="sfg")
    process2.set_red(central_wavelength=1.55e-6,
                     delta_lambda=20e-9,
                     n_points=500)
    process2.set_blue(central_wavelength=0.55e-6,
                      delta_lambda=1e-9,
                      n_points=500)
    process2.calculate_phasematching()
    process2.plot_phasematching()
...
def example2_2D_phasematching():
    ...
    thisnoise = StandardNoise(z=thiswaveguide.z, offset=7.)
    thisnoise.generate_noise(noise_type="logistic", sigma=0.42, k=30 / length)
    noise = np.append(thisnoise.noise, thisnoise.noise[::-1])[::2]
    plt.plot(thiswaveguide.z, noise)
    plt.show()
...
def deltabeta_pm():
    ...
    delta_beta = np.linspace(-0.5, 0.5, 1000)
    this_phasematching = PhasematchingDeltaBeta(z, delta_beta)
    this_phasematching.calculate_phasematching()
    this_phasematching.plot_phasematching()
    plt.show()
...
def different_windowing_functions():
    ...
    this_phasematching = Phasematching1D(waveguide=thiswaveguide,
                                         n_red=ntm,
                                         n_green=ntm,
                                         n_blue=ntm,
                                         process="shg")
    this_phasematching.set_wavelengths(wl_red, wl_red, 0, constlam="shg")
    fig = plt.figure()
    for profile_type in ["constant", "gaussian", "hanning", "bartlett", "hamming", "blackman"]:
        this_phasematching.set_nonlinearity_profile(profile_type=profile_type,
                                                    first_order_coefficient=False,
                                                    sigma_g_norm=0.25)
        this_phasematching.calculate_phasematching()
        this_phasematching.plot(ls="-", lw=1, fig=fig, label=profile_type)
    plt.legend()
    plt.show()


def compare_versions():
    z = np.arange(0, 20, 1e-3)
    delta_beta = np.linspace(-0.5, 0.5, 1000)
    this_phasematching = PhasematchingDeltaBeta(z, delta_beta)
    phi_correct = this_phasematching.calculate_phasematching(old_version=False)
    phi_approximated = this_phasematching.calculate_phasematching(old_version=True)
    phi_formula = z[-1] * np.sinc(delta_beta * z[-1] / 2 / np.pi) * np.exp(1j * delta_beta * z[-1] / 2)
    plt.subplot(121)
    plt.plot(delta_beta, abs(phi_correct) ** 2, label="correct")
    plt.plot(delta_beta, abs(phi_approximated) ** 2, ":", label="approximated")
    plt.plot(delta_beta, abs(phi_formula) ** 2, "-.", label="formula")
    plt.legend()
    plt.subplot(122)
    plt.plot(delta_beta, np.angle(phi_correct), label="correct")
    plt.plot(delta_beta, np.angle(phi_approximated), label="approximated")
    plt.plot(delta_beta, np.angle(phi_formula), "-.", label="formula")
...
if __name__ == "__main__":
    ...
    # example2_2D_phasematching()
    # phasematching_2D()
    # deltabeta_pm()
    # different_windowing_functions()
    # compare_versions()
